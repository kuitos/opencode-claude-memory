...
describe("AutodreamLock", () => {
  test("acquires, blocks a live holder, and releases", () => {
    const path = join(tempDir(), "state", "autodream.lock")
  ...
  test("treats a stale or dead-process lock as free", () => {
    ...
    const stale = new AutodreamLock(
      path,
      () => 1_000 + AUTODREAM_STALE_LOCK_MS,
      222,
      () => true,
    )
    ...
    const dead = new AutodreamLock(
      path,
      () => 2_000 + AUTODREAM_STALE_LOCK_MS,
      333,
      () => false,
    )
  ...
  test("treats an unreadable lock file as free", () => {
    const path = join(tempDir(), "autodream.lock")
...
describe("AutoDream.maybeRun", () => {
  ...
  test("runs a consolidation fork under the dream agent and resets the gate", async () => {
    ...
    expect(existsSync(join(store.stateDir, "autodream.lock"))).toBe(false)
  })
