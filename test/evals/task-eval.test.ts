...
describe("offline task eval harness", () => {
  ...
  test("supports custom judges without changing the harness", async () => {
    const firstCase = TASK_EVAL_CASES[0]
    if (!firstCase) throw new Error("no built-in cases")
    const result = await runTaskEvalCase(firstCase, ({ onPrompt, offPrompt, taskCase }) => {
